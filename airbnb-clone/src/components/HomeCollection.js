...
export default function HomeCollection() {
  ...
  async function fetchDataTourXanh() {
    try {
      const response = await fetch('http://localhost:5001/api/tours?catagoryId=1&_limit=6'); // Gọi API để lấy dữ liệu
  ...
  async function fetchDataTourSinhThai() {
    try {
      const response = await fetch('http://localhost:5001/api/tours?catagoryId=3&_limit=6'); // Gọi API để lấy dữ liệu
      const jsonData = await response.json(); // Chuyển đổi dữ liệu nhận được thành JSON
  ...
  async function fetchDataTourCongDong() {
    try {
      const response = await fetch('http://localhost:5001/api/tours?catagoryId=2&_limit=6'); // Gọi API để lấy dữ liệu
      const jsonData = await response.json(); // Chuyển đổi dữ liệu nhận được thành JSON
